#!/usr/bin/env python3
"""
Builds Joaquin Alec P. Hao's resume PDFs from a single source of truth.

Usage:
    python build_resume.py                 # builds ALL three variants
    python build_resume.py data software   # builds only the named variants

Variants:
    general  -> assets/Hao_Joaquin_Resume_Updated.pdf   (balanced, linked on the site)
    data     -> assets/Hao_Joaquin_Resume_Data.pdf      (data engineering / NetSuite / SQL)
    software -> assets/Hao_Joaquin_Resume_Software.pdf   (full-stack / Next.js / React)

Each variant shares the same experience + projects but tailors the headline,
professional summary, project ordering, and skills ordering to the target track.
"""
...
OUTPUTS = {
    "general":  r"D:\Projects\E-portfolio Website\assets\Hao_Joaquin_Resume_Updated.pdf",
    "data":     r"D:\Projects\E-portfolio Website\assets\Hao_Joaquin_Resume_Data.pdf",
    "software": r"D:\Projects\E-portfolio Website\assets\Hao_Joaquin_Resume_Software.pdf",
}
...
NAME     = S("Name",     fontName="Helvetica-Bold", fontSize=18, alignment=TA_CENTER, spaceAfter=3)
HEADLINE = S("Headline", fontName="Helvetica-Bold", fontSize=10, alignment=TA_CENTER, spaceAfter=3,
             textColor=colors.HexColor("#222222"))
CONTACT  = S("Contact",  fontName="Helvetica",      fontSize=10, alignment=TA_CENTER, spaceAfter=2)
LINKS    = S("Links",    fontName="Helvetica",      fontSize=9.5, alignment=TA_CENTER, spaceAfter=5,
             textColor=colors.HexColor("#333333"))
SECTION  = S("Section",  fontName="Helvetica-Bold", fontSize=11, alignment=TA_CENTER, spaceBefore=5, spaceAfter=2)
ORG      = S("Org",      fontName="Helvetica-Bold", fontSize=10.5, leading=13.5)
ROLE     = S("Role",     fontName="Helvetica",      fontSize=10.5, leading=12.5, spaceAfter=2)
DATE     = S("Date",     fontName="Helvetica",      fontSize=10.5, alignment=TA_RIGHT, leading=13.5)
BODY     = S("Body",     fontName="Helvetica",      fontSize=10.5, leading=13.5, spaceAfter=2)
BULLET   = S("Bullet",   fontName="Helvetica",      fontSize=10.5, leading=13, leftIndent=14, spaceAfter=1)
SUMMARY  = S("Summary",  fontName="Helvetica",      fontSize=10.5, leading=14, alignment=TA_JUSTIFY, spaceAfter=3)
SKILL    = S("Skill",    fontName="Helvetica",      fontSize=10.5, leading=13.5, spaceAfter=2)

# ── Helpers ──────────────────────────────────────────────────────────────────
def hr():
    return HRFlowable(width="100%", thickness=0.5, color=colors.black, spaceAfter=5, spaceBefore=2)
...
def org_row(left_text, right_text, left_bold=True):
    ...
    t = Table([[Paragraph(left_text, ls), Paragraph(right_text, DATE)]], colWidths=[COL_L, COL_R])
...
# ── Header ───────────────────────────────────────────────────────────────────
def header(headline=None):
    items = [
        Paragraph("Joaquin Alec P. Hao", NAME),
    ]
    if headline:
        items.append(Paragraph(headline, HEADLINE))
    items += [
        Paragraph(
            "Filinvest East Homes &#8226; Antipolo City, 1870 &#8226; "
            "joaquinhao@gmail.com &#8226; +63 917 532 2255", CONTACT),
        Paragraph(
            "linkedin.com/in/joaquin-alec-hao &#8226; github.com/MrWack20 &#8226; "
            "e-portfolio-two-eta.vercel.app", LINKS),
        hr(),
    ]
    return items

# ── Headlines + Summaries per variant ────────────────────────────────────────
HEADLINES = {
    "general":  None,
    "data":     "Data Engineer  &#8226;  NetSuite / SuiteQL  &#8226;  SQL &amp; Analytics",
    "software": "Full-Stack Software Engineer  &#8226;  Next.js / React / TypeScript  &#8226;  Supabase",
}

SUMMARIES = {
    "general":
        "Computer Engineering graduate from Jose Rizal University, <b>Cum Laude</b>, with hands-on "
        "experience in full-stack development, data engineering, and AI-integrated systems. Shipped "
        "multiple production applications independently&#8212;including a collaborative AI travel "
        "planner and a real-time card collection manager&#8212;using Next.js 16, Supabase, and Gemini "
        "AI. Proficient at integrating Claude Code into development workflows for accelerated, "
        "production-grade delivery across the full stack. <b>Open to remote opportunities.</b>",
    "data":
        "Computer Engineering graduate from Jose Rizal University, <b>Cum Laude</b>, and a "
        "DataCamp-certified Data Engineer with hands-on experience across the data lifecycle&#8212;"
        "ERP administration, SQL and SuiteQL querying, data cleaning, reconciliation, and governance. "
        "During an enterprise internship, reconciled 7,000+ customer and item records and authored an "
        "ISMS Data Governance Handbook spanning 7 systems (NetSuite, Celigo, Zoho, BigQuery, and more). "
        "Builds data-backed applications end-to-end with PostgreSQL, Supabase, and Firebase. "
        "<b>Open to remote opportunities.</b>",
    "software":
        "Computer Engineering graduate from Jose Rizal University, <b>Cum Laude</b>, and a full-stack "
        "engineer who ships production software solo. Built and deployed VoyageAI&#8212;a collaborative "
        "AI travel-planning SaaS&#8212;end-to-end with Next.js 16, React 19, TypeScript, Supabase "
        "(PostGIS, RLS, Realtime), and the Vercel AI SDK, at zero infrastructure cost. Strong across "
        "the stack: REST APIs, relational and realtime databases, role-based access control, and "
        "AI-assisted delivery with Claude Code. <b>Open to remote opportunities.</b>",
}
    ...
    return [
        KeepTogether([
            ...
            Paragraph(
                ...
                "for Campus Information Management", BODY),
        ]),
        gap(6),
    ]

# ── Experience (chronological for all variants) ──────────────────────────────
def experience_blocks():
    blocks = []
...
# ── Projects (keyed so they can be reordered per variant) ────────────────────
def proj_voyageai():
    return KeepTogether([
...
PROJECT_ORDER = {
    "general":  [proj_voyageai, proj_thesis, proj_pokemon, proj_thrive, proj_library],
    "data":     [proj_pokemon, proj_voyageai, proj_thesis, proj_thrive, proj_library],
    "software": [proj_voyageai, proj_pokemon, proj_thrive, proj_thesis, proj_library],
}

# ── Skills (keyed so they can be reordered per variant) ──────────────────────
SKILLS = {
    "ai":       ("AI &amp; Developer Tools",
                 "Claude Code (AI-assisted development), Vercel AI SDK v6, Gemini AI, Groq, Prompt Engineering, RAG pipelines"),
    "frontend": ("Frontend",
                 "HTML, CSS, TypeScript, ReactJS, Next.js 16, Tailwind CSS"),
    "backend":  ("Backend",
                 "Java, PHP, Python, Next.js App Router, REST APIs"),
    "dbcloud":  ("Database &amp; Cloud",
                 "MySQL, PostgreSQL, Supabase (PostGIS &#8226; RLS &#8226; Realtime), Firebase, AWS, Vercel"),
    "erpdata":  ("ERP &amp; Data Engineering",
                 "NetSuite, SuiteQL, Data Governance, ISMS Documentation, BigQuery"),
    "hardware": ("Hardware &amp; Embedded",
                 "Embedded Systems &amp; IoT, Circuit Design, Robotics, Arduino, ESP32"),
    "interests":("Interests",
                 "Exploring emerging technologies and innovations, gadget tinkering, circuit designing, and robotics development."),
}

SKILL_ORDER = {
    "general":  ["ai", "frontend", "backend", "dbcloud", "erpdata", "hardware", "interests"],
    "data":     ["dbcloud", "erpdata", "ai", "backend", "frontend", "hardware", "interests"],
    "software": ["ai", "frontend", "backend", "dbcloud", "erpdata", "hardware", "interests"],
}

# ── Build one variant ────────────────────────────────────────────────────────
def build(variant):
    story = []
    story += header(HEADLINES[variant])

    story += section("Professional Summary")
    story += [Paragraph(SUMMARIES[variant], SUMMARY)]

    story += section("Education")
    story += education()

    story += section("Experience")
    exp = experience_blocks()
    for i, b in enumerate(exp):
        story.append(b)
        story.append(gap() if i < len(exp) - 1 else gap(5))

    story += section("Projects")
    projs = PROJECT_ORDER[variant]
    for i, p in enumerate(projs):
        story.append(p())
        story.append(gap() if i < len(projs) - 1 else gap(5))

    story += section("Skills &amp; Interests")
    for key in SKILL_ORDER[variant]:
        label, value = SKILLS[key]
    ...
    doc = SimpleDocTemplate(
        OUTPUTS[variant], pagesize=letter,
        leftMargin=MARGIN, rightMargin=MARGIN, topMargin=MARGIN, bottomMargin=MARGIN,
        title="Joaquin Alec P. Hao - Resume", author="Joaquin Alec P. Hao",
    )
    doc.build(story)
    print(f"Done  ->  {OUTPUTS[variant]}")

# ── Entry point ──────────────────────────────────────────────────────────────
if __name__ == "__main__":
    requested = sys.argv[1:] or list(OUTPUTS.keys())
    for v in requested:
        if v not in OUTPUTS:
            print(f"Unknown variant: {v}  (choose from {list(OUTPUTS.keys())})")
            continue
        build(v)
